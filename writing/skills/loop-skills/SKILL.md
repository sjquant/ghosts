...
Frontmatter owns activation. Do not repeat trigger conditions in the body with phrases such as `Use this skill when ...`; begin with direct behavior instructions. State a boundary only as an actionable constraint, such as `If the target is missing, ask for it.`
