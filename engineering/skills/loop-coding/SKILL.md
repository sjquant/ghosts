...
Answer every `Any ...?` row Yes or No in its outcome. On Yes, use the earliest affected state and append a row such as `- [ ] Recheck P2: Run relevant checks` before continuing.
...
Final response: summarize what changed, the checks run, and any remaining uncertainty.
