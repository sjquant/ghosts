...
Generate one concise English handoff for an independent implementation task by default. If the invocation specifies a positive limit such as `max 3`, `maximum 3`, or `최대 3개`, generate up to that many handoffs; treat the limit as a cap. Output each handoff in a code block; do not create a handoff document.
