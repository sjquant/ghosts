...
- Lifecycle contracts: shared setup and teardown prerequisites—such as authentication, connection session creation, or request context—belong at the common entry boundary, not as an implicit first step in every sibling handler. Flag a handler set that repeats an `ensure_*` operation, especially when a missed call silently skips behavior. If a common boundary is infeasible, require the registration site to state the lifecycle and each handler's obligation explicitly.
