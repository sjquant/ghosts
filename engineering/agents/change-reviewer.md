...
- Extension-point contracts: when a sibling handler, plugin, command, job, route, or callback must honor non-obvious prerequisites, ordering, state, or cleanup, enforce that contract at a shared boundary. Flag requirements that remain hidden in sibling implementations or convention: a future addition can compile yet silently malfunction. If shared enforcement is disproportionate, require documentation beside the registration or extension point stating the precondition, why it matters, and the required action.
